  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-white to-slate-50 dark:from-zinc-950 dark:to-zinc-900 text-zinc-900 dark:text-zinc-100">
      ...
      <main>
        ...
        {/* Lightweight sections for anchor targets */}
        <section id="pricing" className="relative py-16 sm:py-20">
          <div className="absolute inset-0 pointer-events-none" style={{
            background: 'radial-gradient(600px 300px at 80% 20%, rgba(59,130,246,0.1), transparent)'
          }} />
          <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <p className="mt-3 text-center text-black/70 dark:text-white/70 max-w-2xl mx-auto">Start free and upgrade when you grow. No hidden fees.</p>
            <div className="mt-10 grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                { name: 'Free', price: '₹0', perks: ['10 farmers', 'Single rate chart', 'Today entries only'] },
                { name: 'Premium', price: '₹200', perks: ['500 farmers', 'PDF export', 'Edit last 1 month'] },
                { name: 'Premium+', price: '₹500', perks: ['Unlimited farmers', 'Offline-first', '1-year editing'] },
              ].map((p) => (
                <div key={p.name} className="rounded-2xl border border-white/40 dark:border-zinc-800 bg-white/60 dark:bg-zinc-900/60 backdrop-blur-xl p-6 shadow-[inset_0_1px_0_rgba(255,255,255,0.5)]">
                  <div className="flex items-baseline justify-between">
                    <h3 className="text-xl font-semibold">{p.name}</h3>
                    <span className="text-2xl font-extrabold">{p.price}<span className="text-sm font-medium opacity-70">/mo</span></span>
                  </div>
                  ...
                  <button className="mt-6 w-full px-4 py-2 rounded-xl bg-gradient-to-r from-indigo-500 via-sky-500 to-emerald-500 text-white hover:opacity-95 transition">Get started</button>
                </div>
              ))}
            </div>
          </div>
